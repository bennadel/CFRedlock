component
	...
	*/
	public any function init() {
		...
		isolatedLockName = "CFRedlockIsolatedServer-#createUUID()#";
	...
	*/
	public boolean function deleteKey( 
		...
		lock 
			type = "exclusive"
			name = isolatedLockName
			timeout = 1
			{

			// If the key doesn't exist, there's nothing to delete.
			if ( ! structKeyExists( keyStore, name ) ) {
			...
			if ( keyStore[ name ].value != value ) {

				return( false );

			}

			structDelete( keyStore, name );
	...
	*/
	public boolean function setKey(
		...
		lock 
			...
			if ( structKeyExists( keyStore, name ) && ( keyStore[ name ].expires > getTickCount() ) ) {
			...
			keyStore[ name ] = {
				value: value,
				expires: ( getTickCount() + ttlInMilliseconds )
			};
